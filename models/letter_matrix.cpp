...
void LetterMatrix::matrix_for_time(Time * _time){
  off();
  light(IL_EST);
  light_hours(_time->get_hours());
  light_minutes(_time->get_minutes());
}
...
void LetterMatrix::light(int * array){
  iterator_y = array[0];
  matrix[iterator_y]= matrix[iterator_y] | array[1];
}

void LetterMatrix::light_hours(int hours){
  if(hours == 12){
    light(MIDI);
  }else{
    light(HOURS[hours%12] );
  }
  light_hour_word(hours);
}

void LetterMatrix::light_minutes(int minutes){
  //gestion mot de liaison
  if(minutes==0) return;
  if(minutes >= 40){
    light(MOINS);
    minutes = 60-minutes;
    if(minutes == 15) light(LE);
  }else{
    light(H_ET);
  }
  //gestion de "demi"
  ...
  if(minutes > 10 && minutes <= 16){ // gestion des minutes de 10 à 16
    light(SEIZE_MINUTES[minutes-10]);
  }else{
...
void LetterMatrix::light_hour_word(int hours){
...
void LetterMatrix::light_minute_word(int minutes){
...
void LetterMatrix::light_minutes_number(int minutes){
    int decimal = minutes/10;
    int unit = minutes%10;
    int * decimal_row = DECIMAL[decimal];
    if(decimal_row) light(decimal_row);
    if(unit > 0){
      if(unit == 1 && decimal_row) light(D_ET);
      light(C_MINUTES[unit-1]);
    }
}
